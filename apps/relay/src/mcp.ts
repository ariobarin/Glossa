...
const MCP_TOOL_COPY = {
  ...
  run_command: {
    ...
    description: "Use this only when the user asked to run tests, builds, Git, or another local project command and the selected workspace reports accessProfile system and permissions.runCommands true. Every new command also requires the complete escaped process input to be shown without truncation and approved in the worker's local terminal; if the input cannot be fully shown or the local user denies it, do not retry or work around the denial. Do not use it for general web research, credential or environment inspection, bypassing file-tool boundaries, or work that structured file tools can perform. Approved commands run with the worker operating-system account's full permissions, inherited environment and credentials, and network access; they are not confined to the exposed root and may affect local or external systems. Inputs that appear to contain access credentials are rejected; if output appears to contain them, the worker suppresses the output and stops the command. Use waitMs 0 for longer commands, or 1500 to 2000 for checks expected to finish near one second. The default is 750 milliseconds.",
  },
  get_command: {
...
const safeWorkerMessages: Record<string, string> = {
  ...
  command_not_approved: "The local user did not approve this system command. Do not retry, rephrase, or bypass the local denial.",
