  ...
  return await withPairingLease(async () => {
    ...
    if (current) {
      try {
        ...
        throw new Error(
          `Glossa could not revoke this computer using its previous relay at ${current.relayOrigin}. Run glossa unpair to clear the local pairing, then run Glossa again. (${reason})`,
        );
      }
      await removeDevice();
    }
