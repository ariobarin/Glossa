...
- `get_command` observes for 30 seconds by default and at most 60 seconds, returning sooner when the command finishes or when a supplied sequence changes;
- only `get_command` uses the MCP SDK's SSE response mode, whose 15-second keepalive comments and anti-buffering headers keep the hosted request active without inventing application progress; its worker routing deadline is the requested observation plus the ordinary relay request budget;
- a legacy worker that did not advertise the longer wait receives at most a 15-second observation during a rolling upgrade;
...
The core protocol uses ordinary MCP tools for command start, status, retained output retrieval, result, and cancellation. The SSE response is only a bounded observation transport and never owns process lifetime. Native MCP Tasks support is deferred until target clients support it dependably and Glossa has durable task state to support their lifecycle.
