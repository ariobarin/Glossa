...
The database stores the device ID, account ID, salt, and scrypt hash. After delivery, the raw device token is stored only on the paired computer. Worker registration authenticates that device token over HTTPS, then returns an opaque worker credential bound to that worker ID and connection generation. Poll, result, heartbeat, and unregister requests use the in-memory worker credential, avoiding repeated database and scrypt work. The relay coalesces durable `last_seen_at` updates to at most once per minute per enrolled device while keeping second-scale liveness in memory. One device can be revoked without affecting the user's other devices or MCP authorizations; revocation removes every active worker credential for that device. Registration repeats durable authentication within its original request deadline if any device revocation overlaps that authentication. A single process-local change marker protects this race without retaining revoked device IDs.

A machine-wide local pairing lease ensures concurrent first-run workspace processes share one computer pairing instead of racing to mint separate credentials. A stored device credential is sufficient for later workspace startups on that computer, so a headless SSH target does not need a browser session or the user's Google/Auth0 refresh token. `glossa unpair` authenticates with the device credential, revokes that device at the relay, and removes the local credential. The revocation request has an eight-second deadline. If revocation cannot be confirmed, explicit unpairing removes the local credential, reports the failure, and leaves server-side cleanup to that relay's device panel. Re-pairing is the explicit way to move a computer to another Glossa account.
...
- temporary command state, including at most 1 MiB of independently retained stdout and stderr per record for bounded range retrieval, packed into at most 64 blocks per stream so tiny writes cannot multiply retained buffer objects, with terminal records limited to five minutes and eight recent records
