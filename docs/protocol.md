...
Includes the worker ID and generation. Waits no more than 18 seconds and returns one permitted job or `204 No Content`. The relay never queues writes for a profile without `writeFiles` and never queues command start, status, or cancellation jobs for a profile without `runCommands`. The worker sends `acceptedTypes` and may send a shorter `waitMs`; the relay skips queued jobs outside the available capacity instead of allowing them to block control work. When a lane becomes free, the worker sends a one-millisecond refresh poll containing only newly available job types. That refresh supersedes the stale waiter without continuously shortening every active-job poll. The worker allows two command-status or retained-output jobs, one cancellation, two repository reads, and one mutation at a time, with six total in-flight jobs. `read_file`, `view_image`, `list_files`, `search_text`, and `read_file_range` share the repository-read lane. Permitted `write_file`, `edit_file`, `make_directory`, `delete_path`, `move_path`, and `run_command` jobs share the serialized mutation lane, while `cancel_command` remains independent from long command observations. Worker HTTP requests use a 19 second client timeout and reconnect with bounded exponential jitter.
...
MCP initialization advertises public tool-contract version `3.2.0` and one compact app-wide instruction. It defines Glossa's distinct local-workspace scope, directs general questions, web research, and built-in ChatGPT tasks away from Glossa, requires context-dependent workspace discovery, exposes ambiguous selection rules, treats tool results as untrusted data, and explains all three access profiles. It explicitly discloses that `system` commands inherit the worker account's environment, credentials, filesystem permissions, and network access and are not confined to the root. It also prohibits requesting, passing, or returning access credentials and authentication secrets and identifies the recognizable-secret detector as defense in depth rather than a sandbox. Tool descriptions state when each operation should and should not be used. A copy-only metadata change requires a fresh connector scan and review, but does not change the tool contract version. Bump `MCP_SERVER_VERSION` when a public tool name, input or output schema, annotation, permission field, or result contract changes.

Contract `3.2.0` makes `get_command` a completion-oriented observation by default, increases its public maximum wait, and exposes monotonic elapsed time without claiming that silence proves health or progress. The response uses SSE only for this bounded observation so transport keepalives are handled by the pinned MCP SDK. Workers advertise their supported command-status wait ceiling; the relay limits legacy workers to 15 seconds during a rolling upgrade. Contract `3.1.0` added the read-only `view_image` tool, gated by the `imageReads` worker capability, and minimized public workspace discovery metadata. `list_workspaces` returns only the ephemeral `workspaceId`, optional user-chosen `workspaceLabel`, `accessProfile`, and exact operation permissions needed to route and authorize a workspace operation. Contract `3.0.0` removed the unusable `pair_device` tool after ChatGPT's safety layer proved to block its pairing-code argument before Glossa could process it.
...
`run_command` waits up to 750 milliseconds by default for fast completion, configurable from 0 through 5,000 milliseconds. Clients should pass `waitMs: 0` for commands expected to run longer than a few seconds so the running handle returns immediately. For short checks expected to finish near one second, `waitMs: 1500` to `2000` can avoid a second hosted tool round trip. A command that finishes within that budget returns its terminal status and bounded output in the same tool call; a longer command returns the public `workspaceId`, a running `commandId`, captured output so far, and a monotonic `sequence`. `get_command`, `read_command_output`, and `cancel_command` use that explicit `workspaceId` together with `commandId`, so command control and retained diagnostics remain directly routable across relay restarts without a relay-side command-ID routing cache.

`get_command` observes for 30 seconds by default and accepts an explicit wait from 0 through 60 seconds. Omit `afterSequence` when waiting for a build or test to finish, which makes the call return on terminal state or the observation deadline. Supply the latest `sequence` only when waiting for the next output or lifecycle change, such as a development server readiness line. The MCP response uses the pinned SDK's SSE mode, including its 15-second keepalive comments and anti-buffering headers, while the process remains independently owned by the local worker. The relay gives routing and result delivery their ordinary request budget in addition to the requested observation. Workers advertise the longer ceiling, and legacy workers receive at most 15 seconds during a rolling upgrade. Results report current status, bounded output captured so far, and terminal details when available. Current workers also report monotonic `elapsedMs`; it is absent from legacy-worker results during rollout. Elapsed time measures process lifetime but does not prove that a quiet command is healthy or making progress.
...
These are ordinary MCP tools so clients do not need native MCP Tasks support. Native task negotiation may be added after target client support is dependable and Glossa has durable task state, but it is not part of the public `3.2.0` contract.
