...
Workers advertise their CLI package version and one integer `protocolVersion`. That version names the complete worker job protocol, including command progress, concurrent jobs, structured reads and mutations, retained command output, and image reads. Registration fails closed when the relay and worker protocol versions differ instead of negotiating combinations of boolean capabilities. The public `list_workspaces` result may still expose descriptive capability booleans for agent guidance; those values are derived from the current protocol rather than negotiated per worker.
