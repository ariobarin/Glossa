...
Full text-file reads and writes are limited to 1 MiB. `view_image` is limited to 4 MiB of compressed PNG, JPEG, or WebP bytes. Structured listings, searches, and ranged reads have the smaller per-call and scan ceilings described above. Returned standard output and standard error share a 12 KiB command-result budget. Each local stream retains a bounded beginning and rolling tail, then the worker allocates the response budget across both streams so noisy standard output cannot erase all diagnostic standard error. A truncated stream therefore preserves useful context from both ends and sets its truncation flag; clients should use `read_command_output` to inspect retained omitted bytes without rerunning the command. Up to four commands may run concurrently per worker; another `run_command` request returns `command_busy` until a slot becomes available. Each command keeps its own output and lifecycle, so completion, timeout, or cancellation does not affect its neighbors. Command starts still share the serialized mutation lane, and cancellation remains independent of a pending command-status wait.
