...
`list_devices` returns an object with `devices`, `availability`, and `message`. It returns one device entry for every active worker. Its `deviceId` is the ephemeral worker identifier accepted by file and command tools, `name` is the enrolled computer name, and `workspaceLabel` is present only when the user explicitly supplied one. Each entry reports `path: "."`; local absolute paths and derived repository names are never returned. When no workers are active, `devices` is empty, `availability` is `"offline"`, and `message` tells the agent that Glossa is the local bridge between ChatGPT and one explicitly exposed workspace, asks it to have the user start or reconnect the local Glossa worker, wait until it appears, and retry, and links to deployment-appropriate official documentation. The managed relay links to `https://glossa.sh/docs/quickstart`; a custom relay links to `https://github.com/ariobarin/glossa/blob/main/docs/self-hosting.md`. When one or more workers are active, `availability` is `"online"` and `message` confirms that workspaces are available. The offline result is a successful, user-safe response rather than a tool error. This preserves the existing MCP input name while allowing several independently routed workspaces on one enrolled computer. Local absolute paths are never transmitted to or returned by the hosted relay.
