...
- Reconnect the ChatGPT integration and verify discovery reports contract `0.1.0-beta.14`, all 11 documented tools, `run_command.waitMs`, and `get_command.deviceId` plus `afterSequence`.
...
Do not submit this packet until the production privacy, terms, and support pages are live, the MCP scan reports contract `0.1.0-beta.14` and matches all 11 documented schemas and annotations, the reviewer account and isolated worker are continuously available, and the unrestricted command-execution decision is resolved.
